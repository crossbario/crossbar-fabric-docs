...
async def main(session):
    ...
    def on_session_join(node_id, worker_id, realm_id, session_details):
        session.log.info('session joined on node "{node_id}", worker "{worker_id}", realm "{realm_id}":\n{session_details}\n',
                         node_id=node_id, worker_id=worker_id, realm_id=realm_id,
    ...
    def on_session_leave(node_id, worker_id, realm_id, session_id):
        session.log.info('session "{session_id}" left on node "{node_id}", worker "{worker_id}", realm "{realm_id}\n"',
                         node_id=node_id, worker_id=worker_id, realm_id=realm_id,
    ...
    monitor_time = 600
    session.log.info('ok, subscribed to session join/leave meta events - now sleeping for {monitor_time} secs ..',
