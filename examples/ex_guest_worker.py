...
async def main(session):
    ...
    for node_id in nodes:
        ...
        if GUEST_ID in workers:
            ...
            session.log.info('Worker "{worker_id}" on node "{node_id}" stopped: {worker_stopped}',
                             node_id=node_id, worker_id=GUEST_ID, worker_stopped=worker_stopped)
        ...
        session.log.info('Worker "{worker_id}" on node "{node_id}" started: {worker_started}',
                         node_id=node_id, worker_id=GUEST_ID, worker_started=worker_started)
    ...
    if False:
        for node_id, worker_id in workers_started:
            ...
            session.log.info('Worker "{worker_id}" on node "{node_id}" stopped: {worker_stopped}',
                             node_id=node_id, worker_stopped=worker_stopped, worker_id=worker_id)
    else:
        session.log.info('The following workers will continue to run: {workers_started}',
