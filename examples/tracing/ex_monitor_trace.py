...
async def main(session):
    ...
    def on_trace_created(trace_id, trace_created):
        session.log.info('Trace "{trace_id}" created:\n{trace_created}',
                         trace_id=trace_id,
                         trace_created=pprint.pformat(trace_created))

    await session.subscribe(on_trace_created, u'crossbarfabriccenter.mrealm.tracing.on_trace_created')

    def on_trace_starting(trace_id):
        session.log.info('Trace "{trace_id}" starting ..',
                         trace_id=trace_id)

    await session.subscribe(on_trace_starting, u'crossbarfabriccenter.mrealm.tracing.on_trace_starting')

    def on_trace_started(trace_id, trace_started):
        session.log.info('Trace "{trace_id}":\n{trace_started}',
                         trace_id=trace_id,
                         trace_started=pprint.pformat(trace_started))

    await session.subscribe(on_trace_started, u'crossbarfabriccenter.mrealm.tracing.on_trace_started')

    def on_trace_stopping(trace_id):
        session.log.info('Trace "{trace_id}" stopping ..',
                         trace_id=trace_id)

    await session.subscribe(on_trace_stopping, u'crossbarfabriccenter.mrealm.tracing.on_trace_stopping')

    def on_trace_stopped(trace_id, trace_stopped):
        session.log.info('Trace "{trace_id}" stopped:\n{trace_stopped}',
                         trace_id=trace_id,
                         trace_stopped=pprint.pformat(trace_stopped))

    await session.subscribe(on_trace_stopped, u'crossbarfabriccenter.mrealm.tracing.on_trace_stopped')

    def on_trace_deleted(trace_id, trace_deleted):
        session.log.info('Trace "{trace_id}" deleted:\n{trace_deleted}',
                         trace_id=trace_id,
                         trace_deleted=pprint.pformat(trace_deleted))

    await session.subscribe(on_trace_deleted, u'crossbarfabriccenter.mrealm.tracing.on_trace_deleted')


    #
    # get IDs of defined traces (to which we are allowed read access)
    #
    trace_ids = await session.call(u'crossbarfabriccenter.mrealm.tracing.get_traces')
    session.log.info('traces defined: {trace_ids}', trace_ids=pprint.pformat(trace_ids))

    #
    # check if our trace exists, and exit if not, or if it is not running ..
    #
    trace = await session.call(u'crossbarfabriccenter.mrealm.tracing.get_trace', TRACE_ID)
    if trace:
        session.log.info('trace "{trace_id}" exists, current status: "{status}"', trace_id=TRACE_ID, status=trace['status'])
        if trace[u'status'] != u'running':
            session.log.info('trace "{trace_id}" exists, but is not running. exiting.', trace_id=TRACE_ID)
            return
    else:
