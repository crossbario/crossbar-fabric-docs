...
async def main(session):
    clusters = await session.call(
        u'crossbarfabriccenter.mrealm.clustering.wsrp.list')
    if CLUSTER_ID not in clusters:
        cluster_created = await session.call(
            u'crossbarfabriccenter.mrealm.clustering.wsrp.create', CLUSTER_ID,
            {})
        session.log.info(
            'cluster "{cluster_id}" created: {cluster_created}',
            cluster_id=CLUSTER_ID,
            cluster_created=cluster_created)
    else:
        session.log.info(
            'cluster "{cluster_id}" already existing', cluster_id=CLUSTER_ID)

    cluster = await session.call(
        u'crossbarfabriccenter.mrealm.clustering.wsrp.get', CLUSTER_ID)
    print(cluster)

    nodes_online = await session.call(
        u'crossbarfabriccenter.mrealm.get_nodes', status=u'online')
    for node_id in CLUSTER_NODES:
        if node_id in nodes_online:
            session.log.info(
                'node "{node_id}" is part of cluster and is online.',
                node_id=node_id)
            ...
            if frontend_id not in cluster[u'frontends']:
                ...
                session.log.info(
                    'frontend "{frontend_id}" already exists - skipping',
                    frontend_id=frontend_id)
