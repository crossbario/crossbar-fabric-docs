...
async def main(session):
    ...
    for node_id in nodes:
        ...
        for worker_id in workers:
            ...
            if worker[u'type'] == u'router':
                ...
                traces = await session.call(u'crossbarfabriccenter.remote.get_router_traces', node_id, worker_id)
                ...
                for trace_id in traces:
                    trace = await session.call(u'crossbarfabriccenter.remote.get_router_trace', node_id, worker_id, trace_id)
                    ...
                    if stop_running and trace[u'status'] == u'running':
                        ...
                        # stop the trace
                        stopped = await session.call(u'crossbarfabriccenter.remote.stop_router_trace', node_id, worker_id, trace_id)

                # start a new trace ..
                trace_id = u'trace-001'
                trace_options = {}
                trace = await session.call(u'crossbarfabriccenter.remote.start_router_trace', node_id, worker_id, trace_id, trace_options=trace_options)
    ...
    for node_id, worker_id, trace_id in started_traces:
        trace_data = await session.call(u'crossbarfabriccenter.remote.get_router_trace_data', node_id, worker_id, trace_id, 0)
        stopped = await session.call(u'crossbarfabriccenter.remote.stop_router_trace', node_id, worker_id, trace_id)
