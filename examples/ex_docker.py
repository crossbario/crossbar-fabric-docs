...
async def main(session):
    ...
    for node_id in nodes:
        ...
        if node_status[u'has_docker']:
            docker_status = await session.call(
                u'crossbarfabriccenter.remote.docker.get_status', node_id)
            session.log.info(
                'Node "{node_id}" has Docker enabled with status: {docker_status}',
                node_id=node_id,
            ...
            for container_id in docker_containers:
                ...
                session.log.info(
                    'Found Docker container: {container_id}',
