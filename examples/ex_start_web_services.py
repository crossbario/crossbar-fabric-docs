        ...
        for i in range(2):
            ...
            for node_id, worker_id in workers_started:
                for path, config in WEB_SERVICES.items():
                    webservice_started = await session.call(u'crossbarfabriccenter.remote.router.start_web_transport_service',
                                                            node_id,
                                                            worker_id,
                                                            TRANSPORT_ID,
                                                            path,
            ...
            for node_id, worker_id in workers_started:
                for path, config in WEB_SERVICES.items():
                    webservice_stopped = await session.call(u'crossbarfabriccenter.remote.router.stop_web_transport_service',
                                                            node_id,
                                                            worker_id,
                                                            TRANSPORT_ID,
                                                            path)
